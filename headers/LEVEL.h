...
struct LEVEL_BLOCK_SIMPLE : public LEVEL_OBJECT
{
	...
	{
	    PosXY nt = PosXY(r.position.X , r.position.Y - LAND_OBJECT::elevation);
	    LO_FLAT_FLOOR* Ls = dynamic_cast< LO_FLAT_FLOOR* >( lastCollision );
	    LO_FLAT_RECT* Lr = dynamic_cast< LO_FLAT_RECT* >( lastCollision );
	    ...
	    else if( Lr!=0 && !Lr->isCollision(nt, prevPos) )
	    {
		r.onGround = false;
		lastCollision = 0;
		return;
